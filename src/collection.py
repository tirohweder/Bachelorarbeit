...
# Prints Table 5.1 / Recorded Transactions per Year / Table 5.3
def table_5_1():
    list1 = ['usdt', 'eth', 'usdc']
    list5 = ['sell', 'buy', 'sell']
...
# Figure 5.2 / Transaction Volume by Month
        ...
        for j in range(total_years[id]):
            ...
            for month in range(12):
                ...
                if (month < 11):
                    ...
                    '{2}-{5}-01' AND side='{3}' '''.format(currency, 2014 + j + year_offset[id],
                                                           2014 + j + year_offset[id], side[id],
                                                           1 + month, 2 + month)

                else:
                    statement = '''SELECT COUNT(*) FROM "hitbtc_trans_{0}" WHERE timestamp BETWEEN '{1}-{4}-01 ' AND 
                    '{2}-{5}-01' AND side='{3}' '''.format(currency, 2014 + j + year_offset[id],
                                                           2015 + j + year_offset[id], side[id],
                                                           1 + month, 1)
...
def plot_month_disto():
    ...
    print(sum(usdt_fitted), sum(eth_fitted), sum(usdc_fitted), sum(usdt_unfitted), sum(eth_unfitted),
          sum(usdc_unfitted))


# Figure 5.3 / Transaction Volume by Weekday
...
# Figure 5.4 / Distribution of Trade Size in USD
def tran_usd_size():
    print("w")
    engine = create_engine('postgresql+psycopg2://trohwede:hallo123@localhost:8877/trohwede')
    conn = engine.connect()
...
# Figure 5.5 / Benfords Law in Traiding Pairs
def benfords_law2():
    engine = create_engine('postgresql+psycopg2://trohwede:hallo123@localhost:8877/trohwede')
    conn = engine.connect()
...
# Figure 5.6 / Occurrence of Quantity in HitBTC Pair
...
# Figure 5.8. Hour Time distribution
def time():
    engine = create_engine('postgresql+psycopg2://trohwede:hallo123@localhost:8877/trohwede')
    conn = engine.connect()
...
# Table 5.8. Detailed view of Unique Matches
def table_5_8_both():
    list1 = ['usdt', 'eth', 'usdc']
    list2 = ['3_2', '2_2', '1_2', '3_0', '2_0', '1_0']
    list3 = [180, 120, 60, 180, 120, 60]
...
# Figure 5.9. Distribution of time
def distribution_of_time():
    engine = create_engine('postgresql+psycopg2://trohwede:hallo123@localhost:8877/trohwede')
    conn = engine.connect()
    ...
    for id, i in enumerate(list1):
        ...
        for idx, j in enumerate(list2):
            ...
            ax[id].plot(df3["index"], a * df3["index"] + b, linestyle='--', c='#899499')
        ...
        ax[id].plot(0, 0, c='#899499', linestyle='--', label='Trendline')
        ax[id].set_ylabel("Number of Matches")
        ax[id].legend()
...
# Figure 5.10. Occurrence of Quantity in Matches
def occurance_parameter_2_vs_02():
    ...
    for id, i in enumerate(list1):
        ...
        for idx, j in enumerate(list2):
            ...
            axs[id][idx].plot(df2.index.values, df2.values, color=color[id], marker='D',
                              label=(list12[id] + " " + list2[
                                  idx]))
            axs[id][idx].legend()
            ...
            axs[id, idx].set_xlabel('Quantity')
            # axs[id, idx].set_ylabel('% of all Transactions')
...
def occurance_qty_by_pair():
    ...
    merged_df_usdt['deposit_transactions'] = (merged_df_usdt['deposit_transactions'] / sum(
        merged_df_usdt['deposit_transactions'])) * 100
    ...
    merged_df_usdc['deposit_transactions'] = (merged_df_usdc['deposit_transactions'] / sum(
        merged_df_usdc['deposit_transactions'])) * 100
    ...
    axs[0, 0].plot(merged_df_usdt['qty'], merged_df_usdt['deposit_transactions'], color='#43a2ca', label='DepTran',
                   alpha=0.8)
    axs[0, 0].set_ylabel('% of all Transactions')
